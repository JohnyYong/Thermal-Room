...
)
{
    ...
    // A very bright ("fully white") surface can come back from PBR above the
    // half-precision range and overflow to Inf in the DX12 build (the Editor
    // tolerates it). Inf/NaN then propagates through the luminance dot and
    // renders as solid BLACK -- which is why only the fully-white surfaces
    // were turning black. Sanitize any non-finite channel, then clamp.
    if (!(color.r < 1e20)) color.r = 1.0;
    if (!(color.g < 1e20)) color.g = 1.0;
    if (!(color.b < 1e20)) color.b = 1.0;
    ...
    // Thermal greyscale is a 0..1 readout; clamp so an over-bright surface
    // saturates to white instead of overflowing.
    grey = saturate(grey);
